...
(()=>{const formatLocalDateTime=(iso,withSeconds)=>{const date=new Date(iso);if(Number.isNaN(date.getTime()))return '';const options={year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false,timeZoneName:'short'};if(withSeconds)options.second='2-digit';return date.toLocaleString(undefined,options)};const formatRelativeReset=(iso)=>{const date=new Date(iso);if(Number.isNaN(date.getTime()))return '';const minutes=Math.max(0,Math.round((date.getTime()-Date.now())/60000));const days=Math.floor(minutes/1440);const hours=Math.floor((minutes%1440)/60);const mins=minutes%60;const parts=[];if(days)parts.push(days+'天');if(hours)parts.push(hours+'小时');if(mins||!parts.length)parts.push(mins+'分钟');return parts.join('')+'后刷新'};document.querySelectorAll('time[data-local-time]').forEach(node=>{const text=formatLocalDateTime(node.dateTime||node.getAttribute('datetime'),node.hasAttribute('data-local-seconds'));if(text)node.textContent=text});document.querySelectorAll('time[data-relative-reset]').forEach(node=>{const text=formatRelativeReset(node.dateTime||node.getAttribute('datetime'));if(text)node.textContent=text});const modal=document.querySelector('[data-add-modal]');const form=document.querySelector('[data-add-form]');const syncButton=document.querySelector('[data-reload-accounts]');const open=()=>modal.hidden=false;const close=()=>modal.hidden=true;const PANEL_ENC_PREFIX_V1='enc::v1::';const PANEL_ENC_PREFIX_V2='enc::v2::';const PANEL_SECRET_SALT='cli-proxy-api-webui::secure-storage';const tryParseJSON=text=>{try{return JSON.parse(text)}catch{return null}};const panelKeyBytes=version=>{try{const suffix=version==='v2'?'|v2|'+window.location.host:'|'+window.location.host+'|'+navigator.userAgent;return new TextEncoder().encode(PANEL_SECRET_SALT+suffix)}catch{return new TextEncoder().encode(PANEL_SECRET_SALT)}};const deobfuscatePanelValue=payload=>{const raw=String(payload==null?'':payload);const prefix=raw.startsWith(PANEL_ENC_PREFIX_V2)?PANEL_ENC_PREFIX_V2:raw.startsWith(PANEL_ENC_PREFIX_V1)?PANEL_ENC_PREFIX_V1:'';if(!prefix)return raw;try{const binary=atob(raw.slice(prefix.length));const encrypted=Uint8Array.from(binary,char=>char.charCodeAt(0));const key=panelKeyBytes(prefix===PANEL_ENC_PREFIX_V2?'v2':'v1');const plain=encrypted.map((byte,index)=>byte^key[index%key.length]);return new TextDecoder().decode(plain)}catch{return raw}};const storageCandidates=()=>{const stores=[];try{stores.push(window.localStorage)}catch{}try{stores.push(window.sessionStorage)}catch{}try{if(window.parent&&window.parent!==window){try{stores.push(window.parent.localStorage)}catch{}try{stores.push(window.parent.sessionStorage)}catch{}}}catch{}return stores};const readStorageItem=(store,name)=>{try{return store&&store.getItem?store.getItem(name):null}catch{return null}};const managementKey=()=>{for(const store of storageCandidates()){const authRaw=readStorageItem(store,'cli-proxy-auth');if(!authRaw)continue;const parsed=tryParseJSON(deobfuscatePanelValue(authRaw));const key=parsed?.state?.managementKey||parsed?.managementKey||'';if(String(key).trim())return String(key).trim()}return''};document.querySelectorAll('[data-close-add]').forEach(button=>button.addEventListener('click',close));const loadManagementPanel=async(view)=>{const key=managementKey();const url=new URL(location.href);url.pathname='/v0/management/plugins/{{.PluginID}}/status';if(typeof view==='string'&&view)url.searchParams.set('view',view);if(!key){alert('未能从 CPA 管理面板读取管理授权，请重新进入管理面板或手动保存。');return}if(syncButton){syncButton.disabled=true;syncButton.textContent='同步中…'}try{const response=await fetch(url,{headers:{Authorization:'Bearer '+key}});const html=await response.text();if(!response.ok)throw new Error('HTTP '+response.status);document.open();document.write(html);document.close()}catch(error){if(syncButton){syncButton.disabled=false;syncButton.textContent='同步 CPA 登录账号'}alert('同步 CPA 登录失败：'+error.message)}};document.querySelectorAll('[data-management-view]').forEach(link=>link.addEventListener('click',event=>{event.preventDefault();loadManagementPanel(link.dataset.managementView||'overview')}));document.querySelectorAll('[data-reload-accounts]').forEach(button=>button.addEventListener('click',loadManagementPanel));document.querySelectorAll('[data-refresh]').forEach(button=>button.addEventListener('click',()=>{button.disabled=true;button.textContent='↻…';loadManagementPanel()}));if(form){const providerInputs=[...form.querySelectorAll('input[name="provider"]')];const arkFields=form.querySelectorAll('.qc-ark-only');const plan=form.elements.plan;const label=form.elements.label;const credential=form.elements.credential;const credentialField=credential&&credential.closest('.qc-field');const submit=form.elements.submit;const presets={zhipu:{plan:'api-Key',planLabel:'智谱 · API Key',label:'智谱主账号',placeholder:'粘贴智谱 API Key，保存后默认遮罩'},minimax:{plan:'coding-plan',planLabel:'MiniMax · 中国站（minimaxi.com）',label:'MiniMax 中国站账号',placeholder:'粘贴 MiniMax API Key，保存后默认遮罩',options:[{label:'MiniMax · 国际站（minimax.io）',endpoint:'https://api.minimax.io/v1/api/openplatform/coding_plan/remains',accountLabel:'MiniMax 国际站账号',placeholder:'粘贴 MiniMax 国际站 API Key，保存后默认遮罩'},{label:'MiniMax · 中国站（minimaxi.com）',endpoint:'https://api.minimaxi.com/v1/token_plan/remains',accountLabel:'MiniMax 中国站账号',placeholder:'粘贴 MiniMax 中国站 API Key，保存后默认遮罩'}]},'opencode-go':{plan:'go',planLabel:'OpenCode Go',label:'OpenCode Go 主账号',placeholder:'粘贴 OpenCode Go Key，保存后默认遮罩'},ark:{plan:'agent-plan',planLabel:'方舟 · Agent Plan',label:'方舟主账号',placeholder:'方舟使用下方 AccessKey ID / Secret'}};let autoLabel='';const updateProvider=(provider,selectedEndpoint)=>{const preset=presets[provider]||presets.zhipu;arkFields.forEach(field=>field.hidden=provider!=='ark');if(credentialField){credentialField.hidden=provider==='ark'}if(plan){if(provider==='minimax'){plan.replaceChildren(...preset.options.map(option=>{const node=new Option(option.label,preset.plan);node.dataset.endpoint=option.endpoint;node.dataset.accountLabel=option.accountLabel;node.dataset.placeholder=option.placeholder;return node}));const wanted=selectedEndpoint||preset.options[1].endpoint;const selected=Array.from(plan.options).find(option=>option.dataset.endpoint===wanted);if(selected)selected.selected=true}else{plan.replaceChildren(new Option(preset.planLabel,preset.plan));plan.value=preset.plan}}const selectedOption=plan&&plan.selectedOptions[0];const generatedLabel=selectedOption?.dataset.accountLabel||preset.label;const generatedPlaceholder=selectedOption?.dataset.placeholder||preset.placeholder;if(label&&(!label.value.trim()||label.value===autoLabel)){label.value=generatedLabel;autoLabel=generatedLabel}if(credential){credential.placeholder=generatedPlaceholder;if(provider==='ark')credential.value=''}if(submit){submit.disabled=false;submit.textContent='验证并添加'}};plan.addEventListener('change',()=>{const selectedOption=plan.selectedOptions[0];if(form.elements.provider.value!=='minimax'||!selectedOption)return;const generatedLabel=selectedOption.dataset.accountLabel||'';if(label&&(!label.value.trim()||label.value===autoLabel)){label.value=generatedLabel;autoLabel=generatedLabel}if(credential&&selectedOption.dataset.placeholder)credential.placeholder=selectedOption.dataset.placeholder});providerInputs.forEach(input=>input.addEventListener('change',()=>updateProvider(input.value)));updateProvider(form.elements.provider.value||'zhipu');const modalTitle=document.querySelector('[data-modal-title]');const accountIdInput=form.elements.account_id;const setModalMode=(mode,preset)=>{if(modalTitle)modalTitle.textContent=mode==='edit'?'编辑供应商连接':'添加供应商连接';if(accountIdInput)accountIdInput.value=preset&&preset.id||'';providerInputs.forEach(input=>{input.disabled=mode==='edit';if(preset&&input.value===preset.provider)input.checked=true;});updateProvider((preset&&preset.provider)||form.elements.provider.value||'zhipu',preset&&preset.endpoint||'');if(label&&preset&&preset.label)label.value=preset.label;if(credential)credential.value='';if(form.elements.access_id)form.elements.access_id.value='';if(form.elements.secret)form.elements.secret.value='';};document.querySelectorAll('[data-open-add]').forEach(button=>button.addEventListener('click',()=>{setModalMode('add');open();}));document.querySelectorAll('[data-edit]').forEach(button=>button.addEventListener('click',()=>{setModalMode('edit',{id:button.getAttribute('data-edit')||'',provider:button.getAttribute('data-provider')||'zhipu',label:button.getAttribute('data-label')||'',endpoint:button.getAttribute('data-endpoint')||''});open();}));document.querySelectorAll('[data-delete]').forEach(button=>button.addEventListener('click',async()=>{const id=button.getAttribute('data-delete')||'';if(!id||!confirm('确定删除这个连接？删除后需要重新添加凭据。'))return;const key=managementKey();if(!key){alert('CPA 管理授权未传入插件窗口，无法删除。');return}const headers={Authorization:'Bearer '+key};button.disabled=true;button.textContent='删除中…';try{let response=await fetch('/v0/management/plugins/{{.PluginID}}/accounts?id='+encodeURIComponent(id),{method:'DELETE',headers});if(response.status===401){alert('CPA 管理授权未传入插件窗口，无法删除。');return}if(!response.ok){let detail='删除失败，请检查管理权限';try{const data=await response.json();if(data&&data.error)detail=String(data.error)}catch{}alert(detail);return}await loadManagementPanel('accounts')}finally{button.disabled=false;button.textContent='删除'}}));form.addEventListener('submit',async event=>{event.preventDefault();const payload={id:form.elements.account_id?.value||'',provider:form.elements.provider.value,plan:form.elements.plan.value,label:form.elements.label.value,credential:form.elements.credential.value,endpoint:plan.selectedOptions[0]?.dataset.endpoint||'',access_id:form.elements.access_id?.value||form.elements.access_key_id?.value||'',secret:form.elements.secret?.value||form.elements.secret_access_key?.value||''};const key=managementKey();if(submit){submit.disabled=true;submit.textContent='验证中…'}try{const response=await fetch('/v0/management/plugins/{{.PluginID}}/accounts',{method:'POST',headers:{'Content-Type':'application/json',...(key?{Authorization:'Bearer '+key}:{})},body:JSON.stringify(payload)});if(response.status===401){alert('CPA 管理授权未传入插件窗口，无法保存；Grok/Codex/Gemini 已登录 CPA 时无需重复保存。');return}if(!response.ok){let detail='保存失败，请检查凭据或管理权限';try{const data=await response.json();if(data&&data.error)detail=String(data.error)}catch{}alert(detail);return}await loadManagementPanel('accounts')}finally{if(submit){submit.disabled=false;submit.textContent='验证并添加'}}})}})();
</script></body></html>`))
